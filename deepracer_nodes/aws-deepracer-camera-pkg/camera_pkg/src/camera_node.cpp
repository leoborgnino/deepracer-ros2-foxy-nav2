...
namespace MediaEng {
    ...
    private:
        ...
        void videoProducerStateHdl(const std::shared_ptr<deepracer_interfaces_pkg::srv::VideoStateSrv::Request> req,
                                   std::shared_ptr<deepracer_interfaces_pkg::srv::VideoStateSrv::Response> res) {
	    
	    RCLCPP_INFO(this->get_logger(), "Request to camera_pkg");
        ...
        void produceFrames() {
			...
			auto custom_header = std_msgs::msg::Header();
			custom_header.frame_id="base_link";
			custom_header.stamp = this->now();
                        msg.images.push_back(*(cv_bridge::CvImage(custom_header, "bgr8", frame).toImageMsg().get()));
                    }
...
int main(int argc, char * argv[])
{
    ...
    std::vector<int> cameraIndex {4, 3, 2, 1, 0};  //{2, 0};//FIXME: Only for my robot, fixme for PR to original {4, 3, 2, 1, 0};
